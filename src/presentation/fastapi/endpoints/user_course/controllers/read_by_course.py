...
class ReadByCourseUserCourseController(IController[UUID4, JsonResponse[UserCoursesInResponse]]):
    ...
    async def __call__(self, id_: UUID4) -> JsonResponse[UserCoursesInResponse]:
        courses = await self.__user_course_repo.read_by_course_id(id_)
        count = await self.__user_course_repo.count_by_course_id(id_)
