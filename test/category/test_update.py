...
async def test_update(
    ...
    response = await app_client.patch(
        f"/api/v1/category/{created_category.get('id')}",
        json=update_data,
    ...
    new_category = response.json().get("answer", {})

    assert created_category.get("id") == new_category.get("id")
    assert new_category.get("title") == must_be.get("title")
    assert new_category.get("description") == must_be.get("description")
